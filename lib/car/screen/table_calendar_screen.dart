...
class TableCalendarScreen extends StatelessWidget {
  ...
  void _showTimePicker(BuildContext context, CalendarController controller, {required bool isStart}) {
    ...
    return Scaffold(
      ...
      body: Consumer<CalendarController>(
        builder: (context, controller, _) {
          ...
          // === 월 목록 생성 ===
          // 오늘부터 90일 후까지 몇 개월에 걸치는지 계산
          // 예: today=4월, maxDay=7월 → (0)*12 + 7 - 4 + 1 = 4개월
          // 예: today=10월(2026), maxDay=1월(2027) → (1)*12 + 1 - 10 + 1 = 4개월
          final monthCount = (maxDay.year - today.year) * 12 + maxDay.month - today.month + 1;
          // 각 달의 1일을 리스트로 생성
          ...
          return Column(
            children: [
              // ▶ 1. 요일 헤더 (상단 고정, 스크롤 안 됨)
              // TableCalendar의 기본 요일 헤더 대신 직접 만들어서
              // 달력 헤더(년월) 위에 배치
              // 일월화수목금토 순서 (TableCalendar 기본 startingDayOfWeek가 일요일)
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 8.0),
                child: Row(
                  // 각 요일을 Expanded로 감싸서 균등 너비 배분
                  // → TableCalendar 내부 날짜 셀과 너비가 맞아짐
                  children: ['일', '월', '화', '수', '목', '금', '토'].map((day) {
                    // 토요일은 파랑, 일요일은 빨강, 나머지는 기본색
                    final color = day == '일' ? Colors.red
                        : day == '토' ? Colors.blue
                        : null;
                    return Expanded(
                      child: Center(
            ...
            itemBuilder: (context, index) {
              final month = months[index]; // 이 달력이 보여줄 달 (예: 2026-04-01)
              ...
              if (controller.rangeStart != null && controller.rangeEnd != null) {
                ...
                // 현재 달이 range 범위 안에 있을 때만 하이라이트 표시
                if (monthYM >= startYM && monthYM <= endYM) {
                  // 시작 달이면 실제 start 날짜에서 시작
                  // 중간 달이면 firstDay(달력 좌측 outside 셀까지 하이라이트 확장)
                  monthRangeStart = (monthYM == startYM) ? start : firstDay;
                  // 끝 달이면 실제 end 날짜에서 끝
                  // 중간 달이면 lastDay(달력 우측 outside 셀까지 하이라이트 확장)
                  monthRangeEnd = (monthYM == endYM) ? end : lastDay;
                }
              ...
              return Padding(
                ...
                enabledDayPredicate: (day) {
                  ...
                  // rangeStart 이후 날짜는 7일까지만 활성화 (최대 숙박일 제한)
                  if (controller.rangeStart != null && controller.rangeEnd == null) {
                    final start = controller.rangeStart!;
                    final maxRange = start.add(const Duration(days: 6));
                    if (day.isAfter(start) && day.isAfter(maxRange)) return false;
                  }
                  return true;
                },

                // === 선택 날짜 표시 조건 ===
                // selectedDayPredicate: 각 날짜 셀에 대해 호출됨
                // true 반환 → selectedDecoration(빨간 원) 적용
                // rangeStart가 있으면 false 반환 → selected 표시 안 함
                // (rangeStart/End 스타일이 우선 적용되도록)
                selectedDayPredicate: (day) {
                  if (controller.rangeStart != null) return false;
                  return day.month == month.month && isSameDay(controller.selectedDay, day);
                },
                ...
                // === 날짜 클릭 이벤트 ===
                // 활성화된(enabled) 날짜를 탭하면 호출됨
                // selectedDay: 사용자가 탭한 날짜
                // focusedDay: 달력의 포커스 날짜 (보통 selectedDay와 동일)
                onDaySelected: (selectedDay, focusedDay) {
                  // outside 날짜(다른 달에 속한 날짜)를 탭해도 반응하지 않게 필터링
                  // 예: 4월 달력에 보이는 3월 31일을 탭하면 무시
                  if (selectedDay.month == month.month && selectedDay.year == month.year) {
                    controller.onDaySelected(selectedDay, focusedDay);
                  }
                },

                // === 달력 표시 옵션 ===
                // 요일 헤더는 Column 최상단에 직접 만들어서 배치했으므로 끔
                ...
                calendarBuilders: CalendarBuilders(
                  ...
                  // (오늘 이전, 90일 이후, rangeStart+7일 이후 등)
                  // 다른 달 날짜면 → SizedBox.expand()로 숨김 (빈 칸 유지)
                  // 현재 달 날짜면 → null 반환 → 기본 회색 글씨로 표시
                  disabledBuilder: (context, day, focusedDay) =>
                  day.month != month.month ? const SizedBox.expand() : null,
                ...
                headerStyle: const HeaderStyle(
                  formatButtonVisible: false, // 월간/주간 전환 버튼 숨김
                  titleCentered: true, // 제목 중앙 정렬
                  leftChevronVisible: false, // ◀ 이전 달 버튼 숨김 (스크롤로 이동하므로 불필요)
                ...
                calendarStyle: const CalendarStyle(
                  ...
                  tablePadding: EdgeInsets.symmetric(horizontal: 16.0),
                  ...
                  rangeHighlightColor: Color(0xFFDDEEFF),
                  ...
                  // range 사이 날짜 글씨 스타일 (기본값 - 특별한 스타일 없음)
                  withinRangeTextStyle: TextStyle(),
                  // range 사이 날짜 모양 (투명 원 - 배경 하이라이트만 보이게)
                        ...
                        children: [
                          Column(
                            ...
                            children: [
                              ...
                              Padding(
                                ...
                                child: Text(controller.rangeStart == null ? '날짜 선택해라' : '${controller.rangeStart!.month}.${controller.rangeStart!.day} ${weekDay[controller.rangeStart!.weekday - 1]}',
                                    style: TextStyle(fontSize: 16.0)),
                              ),
                              GestureDetector(
                                onTap: () => _showTimePicker(context, controller, isStart: true),
                                child: Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Text(controller.startTime ?? '시간 선택', style: TextStyle(fontSize: 16.0)),
                                      SizedBox(width: 8),
                          ...
                          Column(
                            ...
                            children: [
                              ...
                              Padding(
                                padding: const EdgeInsets.fromLTRB(5.0, 0.0, 0.0, 5.0),
                                child: Text(controller.rangeEnd == null ? '날짜 선택해라' : '${controller.rangeEnd!.month}.${controller.rangeEnd!.day} ${weekDay[controller.rangeEnd!.weekday - 1]}',
                                    style: TextStyle(fontSize: 16.0)),
                              ),
                              GestureDetector(
                                onTap: () => _showTimePicker(context, controller, isStart: false),
                                child: Container(
                                  padding: const EdgeInsets.symmetric(horizontal: 12.0, vertical: 10.0),
                                  decoration: BoxDecoration(
                                    border: Border.all(color: Colors.grey.shade400),
                                  ...
                                  child: Row(
                                    mainAxisSize: MainAxisSize.min,
                                    children: [
                                      Text(controller.endTime ?? '시간 선택', style: TextStyle(fontSize: 16.0)),
                                      SizedBox(width: 8),
                                      Icon(Icons.arrow_drop_down, color: Colors.grey),
                                    ],
                                  ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: () {
                          ...
                          final now = DateTime.now();
                          final today = DateTime(now.year, now.month, now.day);
                          if (controller.rangeStart!.year == today.year &&
                              controller.rangeStart!.month == today.month &&
                              controller.rangeStart!.day == today.day) {
                            final time = CalendarController.parseTime(controller.startTime);
                            if (time != null) {
